...
check_response:
	if (ATOMIC_GET(tcm_msg->command_status) != CMD_STATE_IDLE) {
		if (timeout >= CMD_RESPONSE_TIMEOUT_MS) {
			LOGE("Timed out wait for response of command 0x%02x, ATTN level %d, Irq: %s\n",
...
exit:
	...
	if (has_irq_ctrl && in_polling && tcm_dev->hw_if->ops_enable_irq) {
		msleep(POLLING_ENABLE_IRQ_DELAY_MS);/*forbiden mcu irq pull high too low cause irq error*/
